...
public class NanoInjectorController : MonoBehaviour
{
    ...
    void Fire()
    {
        if (GetComponent<PlacementCost>().active && targets.Count > 0)
        {
            if (timer > fireRate)
            {
                ...
                go.GetComponent<NanoInjectorProjectileController>().parent = gameObject;
                go.GetComponent<NanoInjectorProjectileController>().target = targets[0];
            }
        }
