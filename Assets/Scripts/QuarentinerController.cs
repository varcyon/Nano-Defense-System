...
public class QuarentinerController : MonoBehaviour
{
    ...
    void Fire()
    {
        if (GetComponent<PlacementCost>().active && targets.Count > 0)
        {
            if (timer > fireRate)
            {
                ...
                go.GetComponent<QuarentineProjectileController>().parent = gameObject;
                go.GetComponent<QuarentineProjectileController>().target = targets[0];
            }
        }
